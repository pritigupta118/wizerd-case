...
const DesignPreview = ({ configuration }: { configuration: Configuration }) => {
  ...
  const label = MODELS.options.find((supportedModel) => supportedModel.value === model)?.label
  let totalPrize = BASE_PRICE
  if (finish === 'textured') {
    totalPrize += PRODUCT_PRICES.finish.textured
  }
  if (material === 'polycarbonate') {
    totalPrize += PRODUCT_PRICES.material.polycarbonate
  }
  ...
  const {mutate: createPaymentSession} = useMutation({
    ...
    const handleCheckout = () => {
        if (user) {
          ...
          localStorage.setItem("configurationId", id) // this will save the configuaration in local storage
          ...
          <div className='mt-8'>
            <div className='bg-gray-50 p-6 sm:rounded-lg sm:p-8'>
              <div className='flow-root text-sm'>
                ...
                <p className='font-medium text-gray-900'>{formatPrice(totalPrize/100)}</p>
                </div>
              </div>
            </div>
